...
async function userLogin() {
    const loginDetails = await inquirer.prompt([
        ...
        },
        {
            ...
            type: 'password',
            message: 'Enter your password:',
            validate: (input: string) => input ? true : 'Password cannot be empty',
        },
    ]);
...
async function runTypingTest(testDurationInSeconds: number, sampleText: string) {
    ...
    while (!timerExpired) {
        ...
        userInput += typedText + ' ';
        if (userInput.trim() === sampleText) break;
    }
...
async function endTypingTest(elapsedTime: number, sampleText: string, typedText: string) {
...
function calculateWPM(text: string, timeInSeconds: number): number {
